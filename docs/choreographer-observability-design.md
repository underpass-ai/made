...
*Owner: Observability lead · Date: 2026-06-10 · Status: design, pre-implementation*

This document is grounded in the code as it stands on `feat/observability-deliberation-trace`. Every "NEW" metric below names a concrete instrumentation point I verified by reading the source. High-cardinality identifiers (`task_id`, `proposal_id`, `ceremony_id`, `contract_id` on hot paths) are **never** used as metric labels.

---

## 1. Executive summary

**The gap, in one paragraph.** The Choreographer exposes exactly five Prometheus series, hand-rolled in `crates/choreo/src/health.rs::metrics` and fed by a deliberately thin `StatisticsPort` (`record_deliberation(specialty, duration)`, `record_orchestration(duration)`): `choreo_deliberations_total`, `choreo_orchestrations_total`, `choreo_deliberations_specialty_total{specialty}`, `choreo_operation_duration_milliseconds` (a *summary* with only `_sum`/`_count` — no quantiles, no histogram), and `choreo_service_ready` (a NATS-only gauge). There are **no histograms, no error counters, and no signal whatsoever** for the things that make this service interesting: the LLM judge, the providers, the validators, the ceremonies, or deliberation outcome quality. OTEL tracing is wired (`telemetry.rs`, `otel` feature, OTLP/gRPC export) and the `deliberate`/`orchestrate`/`run_ceremony` spans carry useful fields, but the rich per-phase span *events* are emitted through the observer, not the metrics path — so none of it is queryable or alertable. A `NoValidProposal` failure, a judge timeout, a 429 from vLLM, a token-cost blowout, or a ceremony stuck on an unsatisfiable guard are all currently invisible. Critically, judge token usage is **structurally impossible** to read today: `openai_compat::ChatResponse` deserializes only `choices` — the `usage` field is dropped at the wire layer for every provider and the judge.
...
Conventions: histograms use seconds (`_seconds`) for latency; existing series keep their `_milliseconds` names. "NEW" means it does not exist today and requires the instrumentation in §6. All deliberation/judge/validator metrics are recorded through a **new `MetricsRecorderPort`** (see §6), not the existing `StatisticsPort` (which keeps its narrow contract).
...
| `choreo_judge_score` | histogram | `specialty` | `judge.rs::validate` (line 187) or the `attach_validations` loop; the 0.0–1.0 `score` | **NEW** | Distribution/calibration. Clustering at extremes or a flat band signals a broken judge or a wrong threshold. |
...
| Metric | Type | Labels | Instrumentation point | Exists? | Why it matters |
|---|---|---|---|---|---|
| `choreo_ceremony_completed_total` | counter | `ceremony_name`, `outcome` | `execute` return points; `outcome` ∈ {`completed`,`step_failed`,`no_transition`,`iteration_limit`,`already_exists`} from the four error sites (lines 78/127/138/151) | **NEW** | Completion rate per ceremony type and the failure-mode split. `ceremony_name` = `definition_name` (bounded set of YAML definitions), **not** the instance id. |
| `choreo_ceremony_duration_seconds` | histogram | `ceremony_name` | `execute` — `clock.now() − instance.started_at()` at completion | **NEW** | End-to-end meeting latency; per-type p95. |
| `choreo_ceremony_step_duration_seconds` | histogram | `ceremony_name`, `step_id` | `run_step` — clock delta around `execute_handler` (line 201) | **NEW** | Slow-step isolation (a deliberate-step vs a decision-step). `step_id` is a bounded set per definition. |
| `choreo_ceremony_step_total` | counter | `ceremony_name`, `step_id`, `status` | at `step_traces.push` (line 119); `status` from `step_result.status()` | **NEW** | Per-step volume + failure split; denominator for step failure rate. |
| `choreo_ceremony_step_attempts` | histogram | `ceremony_name`, `step_id` | `attempt.get()` returned from `start_step_as` (line 185) | **NEW** | Retry pressure across re-driven runs; high attempts = flaky handler or too-short lease TTL. |
| `choreo_ceremony_transition_blocked_total` | counter | `ceremony_name`, `from_state` | when `next_satisfied_transition` returns `None` and `execute` errors with `no satisfied transition` (line 138) | **NEW** | Distinguishes a deadlocked state machine (guard never satisfiable) from waiting on a pending event. Drop `blocked_guard_names` as a label — guard sets can be large/varied; surface them on the trace instead. |
...
6. **Judge score calibration** — `choreo_judge_score` spread per specialty. A collapsed distribution (all 0.7–0.8) means the judge has lost signal; a bimodal one means proposal quality is genuinely split. Drives threshold tuning.
...
| `CeremonyCompletionLow` | `sum(rate(choreo_ceremony_completed_total{outcome="completed"}[15m])) by (ceremony_name) / sum(rate(choreo_ceremony_completed_total[15m])) by (ceremony_name) < 0.9` | 15m | **ticket** | <90% of a ceremony type reaches terminal — deadlocked guard, flaky step, or unsatisfiable transition. |
| `CeremonyTransitionsDeadlocked` | `sum(rate(choreo_ceremony_transition_blocked_total[5m])) by (ceremony_name, from_state) > 0.05` | 10m | **ticket** | A state can't advance — guard logic deadlock or a missing event. |
...
**Row 6 — Ceremonies**
- *Completion rate* by `ceremony_name`, stacked by `outcome`. Source: `choreo_ceremony_completed_total`. Viz: stacked bar.
- *Ceremony duration* p50/p95 by type. Source: `choreo_ceremony_duration_seconds`.
- *Step duration heatmap* — `choreo_ceremony_step_duration_seconds` over `ceremony_name`×`step_id`. Viz: heatmap.
