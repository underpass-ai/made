# ADR-004: `made-api` is the contract a consumer compiles against

Status: Accepted

## Context

Products that embed the engine have been importing `made-embedded` and, with
it, the domain: `made-core` entities crossing into a consumer's dependency
...
contract 1" — only "I compile against whatever `made-embedded` was that week".

## Decision

`made-api` is a leaf crate holding the published contract of the embedded
...
The crate depends on no `made-*` crate. `made-embedded` depends on it and
implements the trait; the dependency arrow points from implementation to
contract, never back.

`crates/made-api/src` joins the vocabulary gate's guarded paths. It is the
...
- A consumer compiles against `made-api` plus an implementation crate, and is
...
- Everything the engine does beyond this contract remains reachable through
  `made-embedded`'s own methods, unversioned and unpromised. Consumers that
