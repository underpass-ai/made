...
roadmap requirement. Current MADE readiness lives in
[`backlog.md`](./backlog.md) and [`stack-gap-analysis.md`](./stack-gap-analysis.md).

This document defines how `made` could participate in
...
2. add a pre-human expert deliberation stage powered by MADE
...
This is not a proposal to replace PIR wholesale with MADE.
It is a proposal to insert MADE into PIR as a domain-governed
...
MADE, by contrast, is currently a domain-agnostic deliberation service:
...
- MADE becomes a reusable expert-deliberation subsystem inside PIR
...
until MADE is complete enough to behave as a real peer of Runtime and
...
This document does not modify PIR or MADE code.
...
MADE does not currently own any of these.
...
That is exactly why MADE should not replace PIR's domain shell.
If it did, one of two bad things would happen:

1. PIR's domain semantics would be duplicated inside MADE, destroying
   MADE's intended neutrality.
2. MADE would be asked to reason from too-open inputs, violating
...
- MADE owns expert deliberation mechanics
...
### What MADE already has

MADE already provides:
...
### What MADE lacked at this snapshot

At the 2026-04-25 snapshot, this repo's stack-gap analysis said
MADE was not yet a real stack-integrated peer of Runtime and
Kernel.
...
At that time, MADE could host deliberation, but could not yet
...
implemented through MADE, without weakening PIR's bounded domain model.
...
9. if human escalation is required:
   - PIR requests a handoff-report deliberation through MADE
...
- replacing PIR's event catalog with MADE triggers
- replacing PIR's specialist catalog with MADE councils
- moving kernel graph semantics into MADE
- moving runtime governance or execution policy into MADE
- letting MADE emit arbitrary domain events on its own authority
...
That means the MADE integration must not be "give the agents the whole
...
3. PIR maps that bundle into a MADE request
4. MADE returns a structured decision artifact
5. PIR validates and interprets it
6. PIR publishes the next bounded domain event

Event-bus integration between PIR and MADE can exist later, but the
...
They are bounded specialist families owned by PIR, using MADE as the
deliberation engine.
...
New inbound family to MADE via PIR-owned orchestration:
...
5. PIR calls MADE
6. MADE runs the reevaluation council
7. MADE returns a structured result
...
MADE's native model today is freer:
...
For this use case, MADE must support specialist-grade structured
output, meaning:
...
## What "MADE complete" means for this project

The user requirement is correct:

> To start building this, MADE must first be complete.
...
MADE cannot participate in PIR's complex incident flow while still
...
Without this, MADE cannot be trusted as part of a remediation path.
...
- explicit kernel integration boundary in MADE or a PIR-side
  materialization contract that MADE consumes
...
MADE currently only models a subset of that cleanly.
...
If MADE is to sit in a critical incident path, transport claims must
match code.
...
If deployed next to Runtime and Kernel in the stack, MADE cannot expose
...
Before PIR depends on MADE in production, there must be a reproducible
stack proof:
...
## Concrete MADE changes required

This section maps the minimum changes into MADE terms.
...
MADE must be able to carry, at minimum:
...
MADE should not invent prompt assembly ad hoc for PIR.
...
Whether that prompt assembly happens inside PIR or inside MADE is an
implementation choice, but one side must own it explicitly.

### E. Real executor path

Once reevaluation starts emitting remedy events, MADE does not
...
PIR needs a MADE client adapter, likely gRPC.
...
### Phase 0. MADE completion

Do not start PIR integration until MADE closes the current stack gaps.
...
1. add PIR MADE client
2. add one council in MADE for reevaluation
...
### 2. Duplicating domain logic between PIR and MADE

If specialist semantics leak into MADE prompts, validators, and event
...
- MADE receives bounded contracts, not implicit domain knowledge

### 3. Weak traceability across the boundary

If incident ids and causation metadata are not preserved across the PIR →
MADE → PIR round trip, the reevaluation layer becomes opaque.

Mitigation:

- make causal metadata first-class before integration starts

### 4. Premature adoption before MADE is ready

If PIR depends on a still-incomplete MADE, the resulting system will
be less safe than the current direct-human path.

Mitigation:

- treat MADE completion as a hard prerequisite, not a parallel nice-to-have
...
- do this
- but only after MADE reaches stack-complete status
- and do it as a bounded PIR integration, not as PIR replacement

In one sentence:

> MADE should become PIR's expert deliberation engine for complex
...
1. Finish MADE stack-completion work:
   - runtime executor
...
5. Add a PIR-side gRPC client to MADE only after the completion gates above are green
