...
(#108). Metrics, traces, logs and reports project one bounded stream cut
(#110). ADR-012 records the resulting architecture.
...
tests-only #120 supplies the final filled-optional and enum-rotation proof
before this closure merges. Shared domain value objects for list bounds remain
...
projection, embedded OTLP/mTLS wiring and event-plus-registry JSONL sink. G4
and G6 remain future slices.
...
| A8 | **ADR-012 (event sourcing)**: events are the source of truth; snapshots are a cache; the chain seals events; the outbox is a cursor; migration is copy-on-write; supersedes the named sentences of ADR-003 and ADR-009. | ADR review. *[Done in #74: the ADR and phase documentation name the verified implementation and the exact superseded decisions.]* |
