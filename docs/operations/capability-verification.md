...
| `EmbeddedMade::open(path)` | Same Rust facade | No-op unless the host injects a handler | SQLite persists the ceremony event streams, their global order, the folded snapshots and definition publications; mounted definitions and transcripts remain in memory unless replaced |
