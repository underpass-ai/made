...
- All 644 Rust source files have at most one primary public type. The gate also
...
- `made-adapters` keeps configuration, wire DTOs, persistence records and
  runtime state outside the domain. Runtime configuration and request mapping
  are split by responsibility; three large implementation modules remain as
...
The current baseline contains 5 large-file debt entries, down from 70 at the
