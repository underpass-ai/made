...
#
# Builds the `made-stub-llm` binary and ships it in a minimal,
...
# path through MADE's `OpenAiAgent` adapter without a
...
RUN --mount=type=cache,id=cargo-registry-stub-llm,target=/usr/local/cargo/registry \
    ...
    cargo build --release --locked --bin made-stub-llm \
 && install -Dm 0755 target/release/made-stub-llm /out/stub-llm

FROM gcr.io/distroless/cc-debian12:nonroot AS runtime

LABEL org.opencontainers.image.title="made-stub-llm" \
...
COPY --from=builder /out/stub-llm /usr/local/bin/made-stub-llm

USER nonroot:nonroot

EXPOSE 8000

ENTRYPOINT ["/usr/local/bin/made-stub-llm"]
