...
#
# Builds the `made-e2e-provider` binary and ships it in a minimal,
# non-root image. Exercises the `agent-vllm` adapter against a real
# vLLM endpoint — not the full MADE stack — so it has zero
...
RUN --mount=type=cache,id=cargo-registry-e2e-provider,target=/usr/local/cargo/registry \
    ...
    cargo build --release --locked --bin made-e2e-provider \
 && install -Dm 0755 target/release/made-e2e-provider /out/runner

FROM gcr.io/distroless/cc-debian12:nonroot AS runtime

LABEL org.opencontainers.image.title="made-e2e-provider" \
...
COPY --from=builder /out/runner /usr/local/bin/made-e2e-provider

USER nonroot:nonroot

ENTRYPOINT ["/usr/local/bin/made-e2e-provider"]
