...
//! sidecar so the choreographer's `OpenAiAgent` adapter has a real
...
// shape the choreographer's adapter expects.
...
fn stub_report_payload() -> Value {
    json!({
        ...
        "executive_summary": "Stub council report emitted by the choreographer's compose E2E stub-llm sidecar.",
        "findings": [
            {
                "id": "finding-1",
