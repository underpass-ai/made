...
/// A business role inside an agentic system, such as the integrator
/// or the reviewer.
///
...
#[serde(transparent)]
pub struct SystemRoleId(String);

impl SystemRoleId {
    pub fn new(raw: impl AsRef<str>) -> Result<Self, DomainError> {
        slug::slug("system_role_id", raw.as_ref(), MAX_LEN).map(Self)
    }
...
impl fmt::Display for SystemRoleId {
...
impl TryFrom<String> for SystemRoleId {
    type Error = DomainError;
