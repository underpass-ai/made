...
#[serde(transparent)]
pub struct HostTransportRef(String);

impl HostTransportRef {
...
impl fmt::Display for HostTransportRef {
...
mod tests {
    ...
    fn empty_and_oversized_values_are_refused() {
        assert!(HostTransportRef::new("  ").is_err());
        assert!(HostTransportRef::new("x".repeat(MAX_LEN + 1)).is_err());
        assert_eq!(
            HostTransportRef::new("  value  ").unwrap().as_str(),
            "value"
        );
    }
    ...
    fn serde_reuses_constructor_validation() {
        assert!(serde_json::from_str::<HostTransportRef>("\"\"").is_err());
        assert_eq!(
            serde_json::from_str::<HostTransportRef>("\"value\"")
                .unwrap()
                .as_str(),
            "value"
        );
