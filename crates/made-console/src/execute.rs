...
use made_client::v1::{
    ...
    IssueAuthorizationGrantRequest, PauseCeremonyRequest, ResumeCeremonyRequest,
    SearchCeremonyInstancesRequest, StreamCeremonyEndReason,
};
use made_client::{ClientConfig, MadeClient, MadeClientError, ProgressCheckpoint, RequestContext};
use serde_json::json;

use crate::{
    authorization_render, render, Args, ArtifactCommand, AuthorizationCommand, BudgetCommand,
    Command, OutputFormat, ReceiptCommand,
};
...
pub async fn run(args: Args) -> Result<(), MadeClientError> {
    ...
    match args.command {
        ...
        Command::Authorization { command } => {
            authorization(&client, command, args.output).await?;
        }
        Command::Report {
            ceremony_ids,
            title,
    ...
    match command {
        ...
        AuthorizationCommand::Issue {
            ...
            println!(
                "{}",
                render::message(
                    &json!({"version": response.version, "existing": response.existing}),
                    output,
                )
            );
        }
