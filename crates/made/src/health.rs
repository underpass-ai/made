...
async fn metrics(State(state): State<HealthState>) -> Response {
    ...
    body.push_str("# HELP made_deliberations_total Count of deliberations completed.\n");
    body.push_str("# TYPE made_deliberations_total counter\n");
    writeln!(body, "made_deliberations_total {total_deliberations}").unwrap();

    body.push_str("# HELP made_orchestrations_total Count of orchestrations completed.\n");
    body.push_str("# TYPE made_orchestrations_total counter\n");
    writeln!(body, "made_orchestrations_total {total_orchestrations}").unwrap();

    body.push_str(
        "# HELP made_deliberations_specialty_total Count of deliberations per specialty.\n",
    );
    body.push_str("# TYPE made_deliberations_specialty_total counter\n");
    for (specialty, count) in snap.per_specialty() {
        let label = escape_label_value(specialty.as_str());
        writeln!(
            body,
            "made_deliberations_specialty_total{{specialty=\"{label}\"}} {count}"
        )
        .unwrap();
    }

    body.push_str(
        "# HELP made_operation_duration_milliseconds Summed duration across deliberations and orchestrations.\n",
    );
    body.push_str("# TYPE made_operation_duration_milliseconds summary\n");
    writeln!(
        body,
        "made_operation_duration_milliseconds_sum {total_duration_ms}"
    )
    .unwrap();
    writeln!(
        body,
        "made_operation_duration_milliseconds_count {total_ops}"
    )
    .unwrap();

    body.push_str("# HELP made_service_ready 1 when every wired dependency is reachable.\n");
    body.push_str("# TYPE made_service_ready gauge\n");
    writeln!(body, "made_service_ready {ready_gauge}").unwrap();
...
mod tests {
    ...
    use made_adapters::memory::InMemoryStatistics;
    ...
    async fn metrics_emits_prometheus_text_with_zero_counters_on_fresh_boot() {
        ...
        assert!(text.contains("# TYPE made_deliberations_total counter"));
        assert!(text.contains("made_deliberations_total 0"));
        assert!(text.contains("made_orchestrations_total 0"));
        assert!(text.contains("made_operation_duration_milliseconds_sum 0"));
        assert!(text.contains("made_operation_duration_milliseconds_count 0"));
        assert!(text.contains("made_service_ready 1"));
    ...
    async fn metrics_endpoint_renders_recorded_operational_metrics() {
        ...
        use made_core::value_objects::DeliberationOutcome;

        let rec = recorder();
        rec.record_deliberation_outcome(
            &made_core::value_objects::Specialty::new("reviewer").unwrap(),
        ...
        assert!(text.contains("made_service_ready 1"));
        assert!(
            text.contains("made_deliberation_completed_total"),
    ...
    async fn metrics_reflects_recorded_operations_and_specialty_labels() {
        ...
        assert_eq!(resp.status(), StatusCode::OK);
        let text = body_text(resp).await;
        assert!(text.contains("made_deliberations_total 2"));
        assert!(text.contains("made_orchestrations_total 1"));
        assert!(
            text.contains("made_deliberations_specialty_total{specialty=\"triage\"} 1"),
            "missing triage label:\n{text}"
        );
        assert!(
            text.contains("made_deliberations_specialty_total{specialty=\"reviewer\"} 1"),
            "missing reviewer label:\n{text}"
        );
        assert!(text.contains("made_operation_duration_milliseconds_sum 750"));
        assert!(text.contains("made_operation_duration_milliseconds_count 3"));
    }
