...
pub struct OrchestrateUseCase {
    ...
    statistics: Arc<dyn made_core::ports::StatisticsPort>,
    source: String,
}
...
mod tests {
    ...
    use made_core::events::{DeliberationCompletedEvent, PhaseChangedEvent, TaskDispatchedEvent};
    use made_core::ports::{
    ...
    async fn unsuccessful_execution_publishes_task_failed_but_returns_output() {
        ...
        let stats: Arc<dyn made_core::ports::StatisticsPort> = Arc::new(NullStats);
        let usecase =
            OrchestrateUseCase::new(deliberate, executor, bus.clone(), clock, stats, "made");
    ...
    async fn execution_profile_is_merged_with_explicit_execution_options() {
        ...
        let stats: Arc<dyn made_core::ports::StatisticsPort> = Arc::new(NullStats);
        let usecase =
            OrchestrateUseCase::new(deliberate, executor.clone(), bus, clock, stats, "made");
        let metadata = TaskMetadata::new(
            None,
            None,
            None,
