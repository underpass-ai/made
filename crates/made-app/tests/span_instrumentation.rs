...
use made_core::entities::{Council, Deliberation, TaskConstraints, ValidatorReport};
...
impl AgentPort for StubAgent {
    ...
    async fn generate(&self, _: DraftRequest) -> Result<Revision, DomainError> {
        Ok(Revision {
            content: format!("content-{}", self.id.as_str()).into(),
        })
    }
    async fn critique(
        &self,
...
fn task() -> made_core::entities::Task {
    made_core::entities::Task::new(
