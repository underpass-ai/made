...
/// `MADE_REPORT_SCHEMA_PATH` (falling back to the in-repo default)
...
pub async fn run_chain_2(h: &mut Harness, cfg: &HarnessConfig) -> Result<ChainOutcome> {
    let path = std::env::var("MADE_REPORT_SCHEMA_PATH")
...
pub async fn run_chain_2_with_schema(
    ...
    //    it already (via MADE_CONTRACT_DIR), in which case the
