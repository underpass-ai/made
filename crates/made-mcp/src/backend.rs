...
///
/// Constructed via [`MadeMcpGrpcTlsConfig::from_env_for_endpoint`].
...
impl MadeMcpGrpcTlsConfig {
    ...
    /// - explicit `MADE_MCP_GRPC_TLS_MODE` always wins.
    #[must_use]
...
mod tests {
    ...
    fn parse_tls_mode_accepts_aliases() {
        assert_eq!(
            ...
            Some(MadeMcpGrpcTlsMode::Disabled)
        );
        assert_eq!(parse_tls_mode("none"), Some(MadeMcpGrpcTlsMode::Disabled));
        assert_eq!(parse_tls_mode("server"), Some(MadeMcpGrpcTlsMode::Server));
        assert_eq!(parse_tls_mode("tls"), Some(MadeMcpGrpcTlsMode::Server));
        assert_eq!(parse_tls_mode("mutual"), Some(MadeMcpGrpcTlsMode::Mutual));
