use crate::protocol::{
    ...
    ACKNOWLEDGE_CEREMONY_AGENT_INTERVENTION_TOOL, ACKNOWLEDGE_INTEGRATOR_ATTENTION_TOOL,
    ADOPT_EXECUTION_RECEIPT_TOOL, ADVANCE_AGENTIC_SYSTEM_EXECUTION_TOOL,
    APPLY_CEREMONY_TRANSITION_TOOL, APPROVE_CEREMONY_GUARD_TOOL, ASSERT_CEREMONY_REASON_TOOL,
    AWAIT_INTEGRATOR_ATTENTION_TOOL, BEGIN_ARTIFACT_UPLOAD_TOOL, BIND_CEREMONY_INTEGRATOR_TOOL,
    ...
    GET_CEREMONY_INTEGRATOR_BINDING_TOOL, GET_CEREMONY_INTERVENTION_TOOL,
    GET_CEREMONY_TRANSCRIPT_TOOL, GET_EXECUTION_RECEIPT_TOOL, GET_HELP_TOOL, GET_METRICS_TOOL,
    GET_STATUS_TOOL, INSPECT_EXECUTION_RECOVERY_TOOL, INSTANTIATE_AGENTIC_SYSTEM_TOOL,
    LIST_AGENTIC_SYSTEMS_TOOL, LIST_ARTIFACTS_TOOL, LIST_ATTENTION_DELIVERIES_TOOL,
...
pub(crate) const CAPABILITY_GROUPS: &[CapabilityGroup] = &[
    ...
    // After participation and before authorization administration,
    // because the loop is how a host takes part in a whole scope
    // rather than in one exchange: the same delivery ledger, one level
    // up. It is the last group about doing the work; what follows is
    // about who may.
    CapabilityGroup {
        id: "integrator_loop",
        description:
            "Bind one host to a ceremony or a system run, be handed what it is owed, and say what was done about it. Delivery to a host is transport, never evidence that anybody acted.",
        tools: &[
