...
use crate::backend::{MadeMcpGrpcTlsConfig, MadeMcpGrpcTlsMode};
...
pub(crate) async fn open_channel(
    ...
    if tls.mode() != MadeMcpGrpcTlsMode::Disabled {
...
async fn build_client_tls_config(tls: &MadeMcpGrpcTlsConfig) -> Result<ClientTlsConfig, String> {
    ...
    if tls.mode() == MadeMcpGrpcTlsMode::Mutual {
        let cert_path = tls
            .cert_path
            .as_ref()
            .ok_or_else(|| "TLS mode=mutual requires MADE_MCP_GRPC_TLS_CERT_PATH".to_string())?;
        let key_path = tls
            .key_path
            .as_ref()
            .ok_or_else(|| "TLS mode=mutual requires MADE_MCP_GRPC_TLS_KEY_PATH".to_string())?;
