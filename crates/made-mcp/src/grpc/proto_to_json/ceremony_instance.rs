...
pub(crate) fn ceremony_instance_state_to_json(mut state: pb::CeremonyInstanceState) -> Value {
    ...
    json!({
        ...
        "succession": state.succession.as_ref().map(succession_to_json),
        "successor_plan": state.successor_plan.map(succession_plan_to_json),
        "child_groups": state
            .child_groups
            .into_iter()
...
fn succession_to_json(succession: &pb::CeremonySuccessionState) -> Value {
    json!({
...
fn step_state_to_json(step: pb::CeremonyStepState) -> Value {
    json!({
        ...
        "carried_from": step.carried_from.as_ref().map(source_record_ref_to_json),
    })
}
