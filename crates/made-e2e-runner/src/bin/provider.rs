...
//!
//!   MADE_VLLM_ENDPOINT         (required) — e.g. https://.../
//!   MADE_VLLM_MODEL            (required) — e.g. Qwen/Qwen3.5-9B
//!   MADE_VLLM_CLIENT_CERT_PATH (optional) — PEM file
//!   MADE_VLLM_CLIENT_KEY_PATH  (optional) — PEM file
//!   MADE_VLLM_BEARER_TOKEN     (optional) — bearer if no mTLS
//!   MADE_VLLM_MAX_TOKENS       (optional) — default 1024
//!
//! The runner does **not** start MADE. Its job is to pin
//! the adapter-to-vLLM contract. Full end-to-end through
//! MADE (with the vLLM agent registered into a council)
...
async fn main() -> Result<()> {
    ...
    let endpoint = require_env("MADE_VLLM_ENDPOINT")?;
    let model = require_env("MADE_VLLM_MODEL")?;
    let max_tokens = env::var("MADE_VLLM_MAX_TOKENS")
        .ok()
        .map(|raw| raw.parse::<u32>())
        .transpose()
        .context("MADE_VLLM_MAX_TOKENS must parse as u32")?
        .unwrap_or(1024);
    ...
    match (
        env::var("MADE_VLLM_CLIENT_CERT_PATH").ok(),
        env::var("MADE_VLLM_CLIENT_KEY_PATH").ok(),
    ) {
        (Some(cert), Some(key)) => {
        ...
        (None, None) => {}
        (cert, key) => bail!(
            "mTLS misconfigured: MADE_VLLM_CLIENT_CERT_PATH={:?}, MADE_VLLM_CLIENT_KEY_PATH={:?} (both required or neither)",
            cert.is_some(),
            key.is_some(),
        ),
    }

    if let Some(token) = env::var("MADE_VLLM_BEARER_TOKEN")
        .ok()
        .filter(|s| !s.is_empty())
    {
        let bearer =
