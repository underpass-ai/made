...
async fn run_ceremony_scenarios(
    ...
    if selected_scenarios.contains(&E2eScenario::ConcurrentReviewPattern) {
        info!("scenario 17: concurrent-review fans out and reports through public RPC");
        verify_concurrent_review_pattern(client)
            .await
            .context("scenario 17 failed")?;
    }

    if selected_scenarios.contains(&E2eScenario::IncidentReviewPattern) {
        info!("scenario 18: incident_review composes broadcast and bounded maker-checker stages");
        verify_incident_review_pattern(client)
            .await
            .context("scenario 18 failed")?;
    }

    if selected_scenarios.contains(&E2eScenario::DurableChildren) {
        info!("scenario 19: two durable children fan out and rejoin through the recovery cursor");
        verify_durable_children_over_public_rpc(client)
            .await
            .context("scenario 19 failed")?;
    }

    if selected_scenarios.contains(&E2eScenario::CeremonyProgress) {
        info!("scenario 20: live ceremony progress replays, follows, and resumes");
        verify_live_ceremony_progress(client)
            .await
            .context("scenario 20 failed")?;
    }
