//! Map SQLite and serde failures to [`DomainError`].
//!
...
//! here so every SQLite adapter operation logs the original identically and
...
pub(super) fn encoding_failure(error: &serde_json::Error, op: &'static str) -> DomainError {
    tracing::error!(error = %error, operation = op, "SQLite record serde failed");
    DomainError::InvariantViolated {
        reason: "sqlite: stored record could not be encoded or decoded",
    }
}
...
pub(super) fn join_failure(error: &tokio::task::JoinError, op: &'static str) -> DomainError {
    tracing::error!(error = %error, operation = op, "SQLite blocking task failed");
    DomainError::InvariantViolated {
        reason: "sqlite: storage task did not complete",
    }
}
