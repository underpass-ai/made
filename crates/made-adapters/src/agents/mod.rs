...
//! The MADE core is **provider-agnostic**: there is no
...
use made_core::ports::{EvidenceSupportJudgePort, MetricsRecorderPort, ValidatorPort};

/// Build the LLM-judge validator from the environment, when enabled.
///
/// The judge is opt-in via `MADE_JUDGE_ENABLED` (`1`/`true`/`yes`). When
/// enabled it reuses the vLLM endpoint and model (`MADE_VLLM_ENDPOINT`,
/// `MADE_VLLM_MODEL`) and an optional `MADE_JUDGE_THRESHOLD` (default
...
pub fn judge_from_env(
    ...
    #[cfg(not(any(feature = "agent-openai", feature = "agent-vllm")))]
    {
        let _ = metrics;
        tracing::warn!(
            "MADE_JUDGE_ENABLED is set but this build has no Chat-Completions \
...
/// Whether the operator opted the judge in via `MADE_JUDGE_ENABLED`.
fn judge_enabled() -> bool {
    env_flag_enabled("MADE_JUDGE_ENABLED")
}
...
///
/// The support judge is opt-in via `MADE_SUPPORT_JUDGE_ENABLED`
/// (`1`/`true`/`yes`) and reuses the vLLM endpoint and model
/// (`MADE_VLLM_ENDPOINT`, `MADE_VLLM_MODEL`), mirroring
...
pub fn support_judge_from_env(
    ...
    #[cfg(not(any(feature = "agent-openai", feature = "agent-vllm")))]
    {
        let _ = metrics;
        tracing::warn!(
            "MADE_SUPPORT_JUDGE_ENABLED is set but this build has no Chat-Completions \
...
/// `MADE_SUPPORT_JUDGE_ENABLED`.
fn support_judge_enabled() -> bool {
    env_flag_enabled("MADE_SUPPORT_JUDGE_ENABLED")
}
...
fn build_env_support_judge(
    ...
    let endpoint = std::env::var("MADE_VLLM_ENDPOINT").map_err(|_| DomainError::EmptyField {
...
fn build_env_judge(
    ...
    let threshold = std::env::var("MADE_JUDGE_THRESHOLD")
        .ok()
        .map_or(Ok(0.5), |value| {
            value
                .trim()
