...
//!
//! MADE has a single voice across providers: every
...
use made_core::entities::{ExternalContextBundle, TaskConstraints};
use made_core::ports::{Critique, DraftRequest};
use made_core::value_objects::OutputContract;

pub(super) fn system_prompt_generate(id: &str, specialty: &str) -> String {
    format!(
        "You are a specialist agent in MADE.\n\
...
mod tests {
    ...
    fn rubric_serialization_returns_json_for_populated_rubric() {
        use made_core::value_objects::{NumAgents, Rounds, Rubric};
        use serde_json::json;
        use std::collections::BTreeMap;
