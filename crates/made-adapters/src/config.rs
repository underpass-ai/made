...
///
/// Recognised variables (prefixed with `MADE_`):
///
/// | Var                              | Default               |
/// |----------------------------------|-----------------------|
...
/// | `MADE_GRPC_TLS_CLIENT_CA_PATH` | (unset)               |
///
/// `MADE_GRPC_TLS_MODE` accepts `none`, `server`, or `mutual`.
    ...
    pub fn load(&self) -> Result<ServiceConfig, DomainError> {
        ...
        let loaded: Defaults = figment.extract().map_err(|err| {
            ...
            DomainError::InvariantViolated {
                reason: "invalid made environment configuration",
            }
        })?;
