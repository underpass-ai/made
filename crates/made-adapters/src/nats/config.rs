...
//! - publish prefix: `made`
//! - inbound trigger subject wildcard: `made.trigger.>`
...
mod tests {
    ...
    fn subjects_derive_from_prefix() {
        let s = NatsSubjects::new("made", "made.trigger.>").unwrap();
        assert_eq!(s.trigger, "made.trigger.>");
        assert_eq!(s.task_dispatched, "made.task.dispatched");
        assert_eq!(s.task_completed, "made.task.completed");
        assert_eq!(s.task_failed, "made.task.failed");
        assert_eq!(s.deliberation_completed, "made.deliberation.completed");
        assert_eq!(s.phase_changed, "made.phase.changed");
    }

    #[test]
    fn different_prefixes_produce_different_subjects() {
        let prod = NatsSubjects::new("made.prod", "made.prod.trigger.>").unwrap();
        assert_eq!(prod.task_dispatched, "made.prod.task.dispatched");
    }

    #[test]
    fn empty_url_is_rejected() {
        let err = NatsConfig::new("   ", "made", "made.trigger.>").unwrap_err();
        assert!(matches!(err, DomainError::EmptyField { field: "nats.url" }));
    }

    #[test]
    fn empty_prefix_is_rejected() {
        let err = NatsSubjects::new("  ", "made.trigger.>").unwrap_err();
        assert!(matches!(
            err,
    ...
    fn control_characters_in_subject_are_rejected() {
        ...
        let err = NatsSubjects::new("cho\x00reo", "made.trigger.>").unwrap_err();
        assert!(matches!(err, DomainError::InvalidCharacters { .. }));
    }

    #[test]
    fn config_builds_from_defaults() {
        let cfg = NatsConfig::new("nats://nats:4222", "made", "made.trigger.>").unwrap();
        assert_eq!(cfg.url, "nats://nats:4222");
        assert_eq!(cfg.subjects.trigger, "made.trigger.>");
    }
}
