...
use made_app::usecases::DeliberateUseCase;
...
fn task_from(
    ...
    let mut constraints = TaskConstraints::new(
        made_core::value_objects::Rubric::empty(),
        config.rounds(),
        config.num_agents(),
        None,
    );
...
mod tests {
    ...
    use made_app::usecases::DeliberateUseCase;
    use made_core::entities::{CeremonyIntervention, Council};
    use made_core::ports::{ClockPort, CouncilRegistryPort, StatisticsPort};
    use made_core::value_objects::{
    ...
    async fn step_with_output_contract_fails_as_no_valid_proposal_when_violated() {
        ...
        assert!(matches!(
            err,
            made_core::error::DomainError::NoValidProposal { ref contract_id }
    ...
    impl made_core::ports::AgentPort for FabricatingAgent {
        fn id(&self) -> &AgentId {
            &self.id
        }
        ...
        async fn generate(
            &self,
            _request: made_core::ports::DraftRequest,
    ...
    #[async_trait::async_trait]
    impl made_core::ports::AgentPort for GroundedNonsenseAgent {
        fn id(&self) -> &AgentId {
            &self.id
        }
