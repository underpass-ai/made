...
pub fn request(connector: &str, config: serde_json::Value) -> CeremonyExecutionRequest {
    let handler = CeremonyStepHandlerRequest::new(
        CeremonyId::new(format!("c6-{}", uuid::Uuid::new_v4())).unwrap(),
        CeremonyName::new("c6_execution").unwrap(),
        CeremonyVersion::v1(),
...
pub fn scratch() -> tempfile::TempDir {
    let path = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../../tmp/c6-execution");
