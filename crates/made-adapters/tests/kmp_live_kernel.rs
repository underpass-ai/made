...
//!
//! ```text
//! cargo test -p made-adapters --features kmp --test kmp_live_kernel -- --include-ignored
//! ```
//!
...
//!
//! `MADE_KMP_KERNEL_BIN` points at a particular binary; otherwise
...
use made_adapters::kmp::{KernelSessionMemory, StdioKernelTransport, StdioKernelTransportConfig};
...
async fn live_memory() -> (KernelSessionMemory<StdioKernelTransport>, tempfile::TempDir) {
    ...
    if let Ok(binary) = std::env::var("MADE_KMP_KERNEL_BIN") {
    ...
    match StdioKernelTransport::connect(&config).await {
        ...
        Err(error) => panic!(
            "these tests were asked for and no memory kernel could be started: {error}\n\
             ...
             repository — or point MADE_KMP_KERNEL_BIN at one"
        ),
    }
}
