...
async fn trigger_event_over_nats_drives_full_pipeline() {
    ...
    client
        .publish("made.trigger.alert.fired".to_owned(), body.into())
        .await
        .unwrap();
    client.flush().await.unwrap();
