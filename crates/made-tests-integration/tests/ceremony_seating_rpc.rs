...
async fn start(client: &mut MadeServiceClient<tonic::transport::Channel>, id: &str) {
    client
...
async fn a_seated_role_sends_its_work_to_the_panel_the_session_chose() {
    ...
    let seated = made_core::value_objects::Specialty::new("senior_sre_panel").unwrap();
    let agent_id = made_core::value_objects::AgentId::new("agent-senior-sre-0").unwrap();
    fixture
        .agents
        .register(std::sync::Arc::new(made_adapters::noop::NoopAgent::new(
            agent_id.clone(),
            seated.clone(),
        )))
        .await
    ...
    fixture
        .councils
        .register(
            made_core::entities::Council::new(
                made_core::value_objects::CouncilId::new("council-senior-sre").unwrap(),
                seated.clone(),
                vec![agent_id],
