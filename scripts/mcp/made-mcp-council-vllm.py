...
1. made_register_contract
2. made_register_agent, once per vLLM agent
3. made_create_council
4. made_run_council_decision
...
VLLM_ENDPOINT_ENV = "MADE_VLLM_ENDPOINT"
VLLM_MODEL_ENV = "MADE_VLLM_MODEL"
VLLM_AGENT_COUNT_ENV = "MADE_VLLM_AGENT_COUNT"
VLLM_MAX_TOKENS_ENV = "MADE_VLLM_MAX_TOKENS"
VLLM_TIMEOUT_SECS_ENV = "MADE_VLLM_TIMEOUT_SECS"
MCP_BIN_ENV = "MADE_MCP_BIN"
MCP_ENDPOINT_ENV = "MADE_MCP_GRPC_ENDPOINT"
MCP_TOOL_TIMEOUT_ENV = "MADE_MCP_TOOL_TIMEOUT_SECS"
RUN_ID_ENV = "MADE_E2E_RUN_ID"
...
def main(argv: list[str]) -> int:
    try:
        if argv == ["--self-test"]:
            ...
            return 0
        if argv:
            raise E2eError("usage: made-mcp-council-vllm.py [--self-test]")
        run()
        return 0
...
def mcp_command() -> list[str]:
    raw = os.environ.get(MCP_BIN_ENV, "made-mcp").strip()
    if not raw:
...
def report_schema() -> dict[str, Any]:
    return {
        ...
        "$id": "https://underpass.ai/made/e2e/report-minimal.schema.json",
